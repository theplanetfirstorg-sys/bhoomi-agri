...
import { Plus, Sprout, ChevronRight, Loader2, Calendar } from 'lucide-react';
      ...
      {showForm && (
        <div className="card border-leaf-200">
          ...
          <form onSubmit={(e) => { e.preventDefault(); createMutation.mutate(); }} className="space-y-4">
            {/* Step 1: Select farm → plot */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="label">Farm *</label>
                <select
                  className="input"
                  onChange={async (e) => { await loadPlots(e.target.value); setSelectedPlotId(''); }}
                  defaultValue=""
                >
            ...
            return (
              <Link key={crop.id} to={`/crops/${crop.id}`}
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-leaf-50 flex items-center justify-center text-xl">🌿</div>
                  <div>
